...
public class GUITwo extends JFrame implements ActionListener {
	...
	private gamerList gamerList;
	 ...
	 public GUITwo(CompetitorList list, knitterList listKnitters, dancerList listDancers, gamerList listGamer) {
		 
		 this.compList = list;
		 this.knitterList = listKnitters; 
		 this.dancerList = listDancers;
		 this.gamerList = listGamer;
...
//		
			...
			resultList.setText(dancerList.getDancerList());
			//TEST TO CHECK COMBOBOX WORKS
			//resultList.setText("Dancing button test");
		}
		else if (competitionSelected == "Gaming") {
			resultList.setText(gamerList.getGamerList());
			//TEST TO CHECK COMBOBOX WORKS
